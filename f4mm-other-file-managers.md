...
File managers can be added, edited, and removed via the Settings Gui.
Notes:
...
OffsetY=0
```
(if you edit this INI manually, you need to restart F4MM)
(Note: OffsetX and OffsetY are optional settings, if ommitted, 0 (zero) will be used as default value.
...
* __OffsetX__:  
  Optional: horizontal offset (negative=left, positive=right)
* __OffsetY__:  
  Optional: vertical offset (negative=up, positive=down)
...
Pablo Commander - https://www.pablosoftwaresolutions.com/html/pablo_commander.html
```ini
[Pablo Commander]
ProgramExe=Commander.exe
ProgramShortCut=^c
ProgramDelay=0
ProgramSendMethod=Send
Active=1
